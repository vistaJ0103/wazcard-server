...
exports.updateCardView = async (req, res) => {
  ...
  if (cardData) {
    try {
      ...
      const save = await CardView.create({
        cid: cardData.id,
        countryName: cardData.country,
      });
      console.log("cardData", save);
      json(res, 200, "View Updated");
    } catch (error) {
      json(res, 500, error.message);
    }
  } else {
...
exports.getCardViewByCardIdCount = async (req, res) => {
  try {
    const today = new Date();
    const dayOfWeek = today.getDay(); // 0 (Sunday) to 6 (Saturday)
    const diffToMonday = (dayOfWeek + 6) % 7; // Calculate difference to Monday (0 for Monday)
    const mondayThisWeek = new Date(today);
    mondayThisWeek.setDate(today.getDate() - diffToMonday);
    mondayThisWeek.setHours(0, 0, 0, 0); // Set time to start of the day

    const startOfMonth = new Date(today.getFullYear(), today.getMonth(), 1);
    startOfMonth.setHours(0, 0, 0, 0); // Set time to start of the day

    const weeklyData = await Card.findAll({
      where: { uid: req.user.id },
      attributes: ["id", [fn("COUNT", col("card_views.id")), "viewCount"]],
      include: [
        {
          model: CardView,
          attributes: [],
    ...
    const monthlyData = await Card.findAll({
      where: { uid: req.user.id },
      attributes: ["id", [fn("COUNT", col("card_views.id")), "viewCount"]],
      include: [
        {
    ...
    const totalWeeklyViews = weeklyData.reduce(
      (total, card) => total + card.viewCount,
      0
    );
    const totalMonthlyViews = monthlyData.reduce(
      (total, card) => total + card.viewCount,
      0
    );

    json(res, 200, null, { totalWeeklyViews, totalMonthlyViews });
  } catch (error) {
    json(res, 500, error.message);
  }
};
...
exports.getCardViewByCountryCount = async (req, res) => {
  ...
  if (cardData) {
    try {
      ...
      json(res, 500, error.message, "werwer");
    }
  } else {
...
exports.toggleCardBill = async (req, res) => {
  try {
    let check = await Card.findByPk(req.body.id);
    if (check) {
      await Card.update(
      ...
      json(res, 200, "Billed successfully");
    } else {
      json(res, 404, "Card not found");
    }
  } catch (error) {
